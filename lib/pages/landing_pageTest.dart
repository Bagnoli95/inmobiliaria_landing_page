...
import 'package:inmobiliaria_app/widgets/home_banner_section.dart';
import 'package:inmobiliaria_app/widgets/property_categories_section.dart';
import 'package:inmobiliaria_app/widgets/testimonials_section.dart';
import 'package:inmobiliaria_app/widgets/why_choose_us_section.dart';

class LandingPageT extends StatefulWidget {
  const LandingPageT({Key? key}) : super(key: key);
...
class _LandingPageTState extends State<LandingPageT> {
  ...
  void _navigateToProperties(String category) {
      ...
      body: Stack(
        children: [
          ...
          SingleChildScrollView(
            ...
            child: Column(
              children: [
                _buildNavBar(isSmallScreen),

                // Banner principal inspirado en Proinvest
                HomeBannerSection(onExploreProperties: () => _scrollToSection(_categoriesKey)),

                // Categorías de propiedades
                PropertyCategoriesSection(key: _categoriesKey, onCategorySelected: _navigateToProperties),

                // Sección de características destacadas
                FeatureSection(key: _featuresKey),

                // Sección "Por qué elegirnos"
                WhyChooseUsSection(key: _whyChooseUsKey),

                // Sección de propiedad destacada con visualizador interactivo
                _buildHeroSection(isSmallScreen, key: _heroKey),
      ...
      children: [
        Row(
          children: [
            ...
            Container(height: 20, width: 1, color: AppColors.primary.withOpacity(0.3), margin: const EdgeInsets.symmetric(horizontal: 15)),
            Text('PREMIUM', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: AppColors.accent, letterSpacing: 1.5)),
          ],
        ),
        Row(
          children: [
            _navButton('Inicio', () => _scrollToTopButton()),
            _navButton('Propiedades', () => _scrollToSection(_categoriesKey)),
            _navButton('Características', () => _scrollToSection(_featuresKey)),
            _navButton('Nosotros', () => _scrollToSection(_whyChooseUsKey)),
            _navButton('Galería', () => _scrollToSection(_galleryKey)),
            _navButton('Testimonios', () => _scrollToSection(_testimonialsKey)),
          ...
          children: [
            DrawerHeader(
              ...
              child: Column(
                ...
                children: const [Text('INMOBILIARIA', style: TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold, letterSpacing: 2.0)), SizedBox(height: 8), Text('PROPIEDADES EXCLUSIVAS', style: TextStyle(color: Colors.white70, fontSize: 12, letterSpacing: 1.5))],
              ),
            ),
            ...
            _buildDrawerItem('Por qué elegirnos', Icons.verified_outlined, () {
              Navigator.pop(context);
              _scrollToSection(_whyChooseUsKey);
            }),
            _buildDrawerItem('Propiedad Destacada', Icons.villa_outlined, () {
              Navigator.pop(context);
              _scrollToSection(_heroKey);
            }),
            _buildDrawerItem('Testimonios', Icons.format_quote_outlined, () {
              Navigator.pop(context);
